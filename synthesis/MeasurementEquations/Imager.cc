...
// Compsens algorithm
Bool Imager::compsens(const String& algorithm,
		   const Int niter, 
		   const Float gain,
const String& minimization,
const String& transform,
const Float ksigma,
const Int Nbscales,
const Bool Positivity,
const Bool no_coarse,
const Float TolVar,
const Bool verbose,
const Bool do_cs_convolve,
const Bool AllOutput,
		   const Quantity& threshold, 
		   const Bool /*displayProgress*/,
		   const Vector<String>& model, const Vector<Bool>& fixed,
		   const String& complist,
...
{
#ifdef PABLO_IO
  traceEvent(1,"Entering Imager::compsens",22);
#endif
  Bool converged=True; 
...
#ifdef PABLO_IO
      traceEvent(1,"Exiting Imager::compsens",21);
#endif

      return False;
    }
  logSink_p.clearLocally();
  LogIO os(LogOrigin("imager", "compsens()"),logSink_p);
  ...
  try {
    ...
    // Make first image with the required shape and coordinates only if
    // it doesn't exist yet. Otherwise we'll throw an exception later
    if(modelNames(0)=="") modelNames(0)=imageName()+".compsens";
    if(!Table::isWritable(modelNames(0))) {
      make(modelNames(0));
    }
    else{
    ...
    if(sm_p){
      if( sm_p->getAlgorithm() != "compsens") destroySkyEquation();
      if(images_p.nelements() != uInt(nmodels)){
	destroySkyEquation();
      }
      else{
...
else if (algorithm=="compsens") {
sm_p = new CoSeImageSkyModel();
        ((CoSeImageSkyModel*)sm_p)->setMinimization(minimization);
        ((CoSeImageSkyModel*)sm_p)->setTransform(transform);
        ((CoSeImageSkyModel*)sm_p)->setkSigma(ksigma);
        ((CoSeImageSkyModel*)sm_p)->setNbScales(Nbscales);
        ((CoSeImageSkyModel*)sm_p)->setPositivity(Positivity);
        ((CoSeImageSkyModel*)sm_p)->setNoCoarse(no_coarse);
        ((CoSeImageSkyModel*)sm_p)->setTolVar(TolVar);
        ((CoSeImageSkyModel*)sm_p)->setVerbose(verbose); // verbose already defined before
        ((CoSeImageSkyModel*)sm_p)->setdoConvolve(do_cs_convolve); 
        ((CoSeImageSkyModel*)sm_p)->setAllOutput(AllOutput);

os << LogIO::NORMAL << "Using Compressed Sensing" << LogIO::POST; //Loglevel INFO
}
    ...
    {
      ostringstream oos;
      oos << "CompSens gain = " <<gain<<", Niter = "<<niter<<", Threshold = "
	  << threshold;
      os << LogIO::NORMAL << String(oos) << LogIO::POST; // More for the
                                                         // logfile than the
                                                         // log window.
    ...
    savePSF(psfnames);
    redoSkyModel_p=False;
    restoreImages(image,do_cs_convolve);
    writeFluxScales(fluxscale_p);
    this->writeHistory(os);
    try{
     // write data processing history into image logtable
  ...
  os << LogIO::NORMAL << "Exiting Imager::compsens" << LogIO::POST; // Loglevel PROGRESS
  return converged;
}
