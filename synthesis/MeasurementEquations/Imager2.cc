...
Bool Imager::restoreImages(const Vector<String>& restoredNames, Bool do_convolve)
{
    ...
    */
	...
	if(restoredNames(thismodel)!="") {
	  ...
	  TempImage<Float> restored;
           ...
           double sum = 0; IPosition pos(4, 0, 0, 0, 0);
           for (int i=0; i<restored.shape()[0]; ++i) for (int j=0; j<restored.shape()[1]; ++j) {
             pos[0] = i; pos[1] = j; sum += restored.getAt(pos);
           }
           StokesImageUtil::Convolve(restored, bmaj_p, bmin_p, bpa_p);
           if ( !do_convolve ) {   // CS - undo the convolution but tell us how much to scale
             double new_sum = 0;
             for (int i=0; i<restored.shape()[0]; ++i) for (int j=0; j<restored.shape()[1]; ++j) {
               pos[0] = i; pos[1] = j; new_sum += restored.getAt(pos);
             }
             restored=TempImage<Float>(modelIm.shape(),
                               modelIm.coordinates());
             restored.copyData(modelIm);
             for (int i=0; i<restored.shape()[0]; ++i) for (int j=0; j<restored.shape()[1]; ++j) {
               pos[0] = i; pos[1] = j; restored.putAt(restored.getAt(pos)*new_sum/sum, pos);
             }
             cout << "Beam scale: " << new_sum/sum << endl;
           }
         }
