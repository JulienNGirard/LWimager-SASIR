...
int main (Int argc, char** argv)
{
  try {
    ...
    inputs.create ("operation", "image",
		   "Operation (empty,image,psf,clark,hogbom,csclean,compsens,multiscale,entropy)",
		   "string");
    inputs.create ("niter", "1000",
		   "Number of clean iterations",
		   "int");
    inputs.create ("gain", "0.1",
		   "Loop gain for cleaning, [CS] gain of relaxation parameter (mu)",
		   "float");
    inputs.create ("threshold", "0Jy",
		   "Flux level at which to stop cleaning [CS] regularization parameter (lambda)",
		   "quantity string");
    ...
    inputs.create ("stoppointmode", "-1",
                   ...
                   "int");
    inputs.create("minimization", "fista",
                  "[CS] Compressed sensing algorithm to run (fista,mca)",
		  "string");
    inputs.create("transform", "wavelets",
                  "[CS] Compressed sensing dictionary for image reconstruction",
                  "string");
    inputs.create ("convolve", "False",
                   "[CS] Convolve the compressed sensing output with the clean beam",
                   "bool");
    inputs.create ("ksigma", "0",
                   "[CS] If set > 0, turns on MAD thresholding for compressed sensing with ksigma equal to this value. Threshold is ignored.",
                   "double");
    inputs.create("Nbscales", "7",
                        "[CS] Number of scales of the transform (be careful for Nscale >7, may not be implemented for IUWT).",
                "int");
    inputs.create("Positivity", "True",
                        "[CS] Enable the positivity constraint",
                "bool");
    inputs.create("TolVar", "0.000001",
                        "[CS] Stopping criterion of the Compressed Sensing",
                "float");
    inputs.create("NoCoarse", "False",
                        "[CS] To remove the coarse scale of the transform",
                "bool");
    inputs.create("verbose", "True",
                        "[CS] Verbose mode for the Compressed Sensing",
                "bool");

    inputs.create("AllOutput", "False",
                        "[CS] Save output image at each iteration along with each scale given by transform.",
                "bool");
...
// CS
    String minimization = inputs.getString("minimization");
    String transform = inputs.getString("transform");
    Double ksigma    = inputs.getDouble("ksigma");
    Int Nbscales  = inputs.getInt("Nbscales");
    Bool Positivity  = inputs.getBool("Positivity");
    Bool no_coarse   = inputs.getBool("NoCoarse");
    double TolVar     = inputs.getDouble("TolVar");
    Bool verbose   = inputs.getBool("verbose");
    Bool do_cs_convolve = inputs.getBool("convolve");
    Bool AllOutput = inputs.getBool("AllOutput");
// CS
    ...
    AlwaysAssertExit (operation=="empty" || operation=="image" ||
                      ...
                      operation=="clark" || operation=="csclean" || operation == "compsens" ||
                      operation=="multiscale" || operation =="entropy");
    AlwaysAssertExit(minimization=="fista" || minimization=="mca");
    AlwaysAssertExit(transform=="wavelets" || transform=="curvelets");
